...
$coinsured = $_POST['coinsured'] ?? '';

$coinsured_company = $_POST['coinsured_company'] ?? '';

$coinsured_street = $_POST['coinsured_street'] ?? '';

$coinsured_postalcode = $_POST['coinsured_postalcode'] ?? '';

$coinsured_place = $_POST['coinsured_place'] ?? '';

$coinsured_function = $_POST['coinsured_function'] ?? '';
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($stmt->execute()) {
        ...
        insertRestOfColumns($last_id);

        if ($isCompleted){
            sendEmailInCompleted($_POST['applicant_email'], $last_id);
            sendEmailInCompleted('service@surendo.com', $last_id);
            $_SESSION['is_completed'] = true;
        }

        $_SESSION['success_message'] = "Ihre Daten wurden erfolgreich übermittelt. Ihre Projekt-ID lautet: <strong>$last_id</strong>";

        if (!(empty($password))) {

            $_SESSION['password'] = $password;

            $_SESSION['new_email'] = $applicant_email;
...
function insertRestOfColumns($pv_plant_id)
{
    global $conn;
    ...
    if ($stmt->execute()) {
        ...
        die("Error inserting pv_plant_1 data: " . $stmt->error);
    }

}

function sendEmailInCompleted($email, $pvPlantId)
{
    ...
    <div style="font-family: Arial, sans-serif; background-color: #cfe9ee7c; padding: 20px; border: 0px; border-radius: 12px; max-width: 500px; margin: 0 auto;">
        ...
        <p style="font-size: 16px; color: #333; text-align: center;">
            Ihr Projekt mit der ID-Nummer ' . htmlspecialchars($pvPlantId) . ' wurde erfolgreich abgeschlossen.
        </p>
