...
<div><h2>1. Antragsteller/in (Versicherungsnehmer/in)</h2></div>
...
<div class="subsection"><b>Antragsteller/in im Verhältnis zur Anlage</b></div>
        ...
        <input type="radio" id="option1" name="relation_to_plant" value="operator_owner" checked> <!-- name ist der DB-Parameter --> 
...
<div class="toggle-content-applicant content-operator-foreign-plant">
    ...
    <div class="relation_to_plant_style"><label><b>Eigentümer/in der Anlage</b> (keine automatische Mitversicherung - diese muss unter Abschnitt Mitversicherung beantragt werden)</label></div>
    <br>
    <label class="label-classic">Firmenname</label>
...
<div class="toggle-content-applicant content-owner-only">
    ...
    <div class="relation_to_plant_style"><label><b>Betreiber/in der Anlage</b> (keine automatische Mitversicherung - diese muss unter Abschnitt Mitversicherung beantragt werden)</label></div>
    <br>
    <label class="label-classic">Firmenname</label>
...
<div><h2>2. Versicherungsort</h2></div>
...
<div class="subsection"><b>Address- oder Koordinateneingabe:</b></div>
            ...
            <!-- Toggle-Buttons JA/NEIN -->
            <input type="radio" id="option1_address" name="address_or_coordinates" value="address" checked> <!-- name ist der DB-Parameter --> 
...
<div class="toggle-content-address-coordinates content-address">
    ...
    <br>
    <label class="label-classic">Postleitzahl</label>
    <input type="text" name="address_postalcode" id="address_postalcode" maxlength="8">
    <br>
    <label class="label-classic">Ort</label>
...
<div class="toggle-content-insured-land">
    ...
    <br><br>
    <label class="label-classic">Die Gesellschaft des Antragssteller/-s/-in befindet sich zu mindestens 50% in österreichischem Besitz (direkt oder indirekt)</label>
...
<br><br>
        <label class="label-classic"><b>Sind die PV-Module im österreichischen <a href="https://www.hagelregister.at" target="_blank" rel="noopener noreferrer">Hagelschutzregister</a> eingetragen?</b></label>
...
<br><br>
        ...
        <option value="" disabled selected hidden>Bitte wählen</option>
        <option value="Ja">Ja</option>
        <option value="Nein. Die PV-Anlage ist im Bau befindlich bzw. noch nicht fertiggestellt.">Nein. Die PV-Anlage ist im Bau befindlich bzw. noch nicht fertiggestellt.</option>
        </select>
<br><br>
        <label class="label-classic"><b>Fällt irgendwo Schatten auf die Module der Anlage? Durch die (Teil)- Verschattung ist die Bildung von Hot-Spots möglich.</b></label>
...
<br><br>
        <label class="label-classic"><b>Ist der Zustand des Untergrunds der PV-Anlage (Dachflächen, Grund und Boden) frei von bekannten Schäden, Mängeln, Schadstoffen oder Umwelt-Altlasten?</b></label> 
...
<!-- VERSICHERUNGSBEGINN - -->
<div><h2>3. Versicherungsbeginn</h2></div>
...
<label class="label-classic" for="date_commencement">Versicherungsbeginn (keine Deckung vor Eingang des Antrags beim Versicherer):</label>
        ...
        <input type="date" id="date_commencement" name="date_commencement" min="<?= htmlspecialchars(date('Y-m-d')) ?>" value="<?= htmlspecialchars(date('Y-m-d')) ?>" required>
        <!-- Fallback für ältere Browser muss evtl. noch programmiert werden -->
<br>
</div>
<hr class="blue-line"> <!-- Blaue Linie eingefügt -->
<div class="subsection-save"> 
    <span class="small-text">Sichern Sie den aktuellen Stand Ihrer Eingaben, z.B. um zu einem späteren Zeitpunkt damit fortzufahren.</span>
    <button class="save-btn">Speichern</button>
</div>
<hr class="blue-line"> <!-- Blaue Linie eingefügt -->

<!-- TECHNISCHE DATEN DER PV-ANLAGE - -->
<div><h2>4. Technische Daten der PV-Anlage</h2></div>
...
<br>
<span class="small-text">Bitte "0" eingeben, wenn Komponente nicht Bestandteil der Anlage ist</span></div>

<div class="form-container"> <!-- Container damit Input-Felder Zahlen linksbündig angeordnet werden --> 
<label class="label-classic">Module (€)</label>
<input type="number" id="eur_panels" name="eur_panels" step="0.01" oninput="calculateTotal()" value="<?php echo isset($_POST['eur_panels']) ? number_format($_POST['eur_panels'], 2, ',', '.') : ''; ?>" required>

<label class="label-classic">Wechselrichter (€)</label>
<input type="number" id="eur_inverter" name="eur_inverter" step="0.01" oninput="calculateTotal()" value="<?php echo isset($_POST['eur_inverter']) ? number_format($_POST['eur_inverter'], 2, ',', '.') : ''; ?>" required>

<label class="label-classic">Transformator NUR für die PV-Anlage; nur dann ist der Trafo versicherbar (€)</label>
<input type="number" id="eur_transformer" name="eur_transformer" step="0.01" oninput="calculateTotal()" value="<?php echo isset($_POST['eur_transformer']) ? number_format($_POST['eur_transformer'], 2, ',', '.') : ''; ?>" required>

<label class="label-classic">Tragkonstruktion (€)</label>
<input type="number" id="eur_supporting_structure" name="eur_supporting_structure" step="0.01" oninput="calculateTotal()" value="<?php echo isset($_POST['eur_supporting_structure']) ? number_format($_POST['eur_supporting_structure'], 2, ',', '.') : ''; ?>" required>

<label class="label-classic">24/7-Videoüberwachung mit Bewegungsmelder (€)</label>
<input type="number" id="eur_video" name="eur_video" step="0.01" oninput="calculateTotal()" value="<?php echo isset($_POST['eur_video']) ? number_format($_POST['eur_video'], 2, ',', '.') : ''; ?>" required>

<label class="label-classic">Zaun/ Umzäunung; kein Mobilzaun! (€)</label>
<input type="number" id="eur_fence" name="eur_fence" step="0.01" oninput="calculateTotal()" value="<?php echo isset($_POST['eur_fence']) ? number_format($_POST['eur_fence'], 2, ',', '.') : ''; ?>" required>

<label class="label-classic">Sonstige bzw. Sammelposition für zuvor nicht genannte Komponenten (€)</label>
<input type="number" id="eur_miscellaneous" name="eur_miscellaneous" step="0.01" oninput="calculateTotal()" value="<?php echo isset($_POST['eur_miscellaneous']) ? number_format($_POST['eur_miscellaneous'], 2, ',', '.') : ''; ?>" required>
</div>
...
<div><h2>5. Ertragsausfall- bzw. Betriebsunterbrechungsversicherung</h2></div>

<!-- BU STROM-VERKAUF -->
<label class="label-classic">Soll der Ertragsausfall des eingespeisten (verkauften) Stroms versichert werden?</label>
<br>
...
<div class="toggle-content-bi-sale">
    ...
    <div class="subsection"><b>Strom-Verkauf (bei Einspeisung)</b></div>
        ...
        <input type="number" id="BI_annual_feed" name="BI_annual_feed" step="0.01" oninput="calculate_BI_Total()" value="<?php echo isset($_POST['BI_annual_feed']) ? number_format($_POST['BI_annual_feed'], 2, ',', '.') : ''; ?>">

        <label>x Einspeisevergütung gemäß EVU-Vertrag in € / kWh</label>
        <input type="number" id="BI_feed_in_tariff" name="BI_feed_in_tariff" step="0.01" oninput="calculate_BI_Total()" value="<?php echo isset($_POST['BI_feed_in_tariff']) ? number_format($_POST['BI_feed_in_tariff'], 2, ',', '.') : ''; ?>">
    </div>
</div>
<br>
...
<div class="toggle-content-bi-self">
    ...
    <div class="subsection"><b>Strom-Zukauf (Eigenverbrauch)</b></div>
        ...
        <input type="number" id="BI_annual_self_consumption" name="BI_annual_self_consumption" step="0.01" oninput="calculate_BI_Total()" value="<?php echo isset($_POST['BI_annual_self_consumption']) ? number_format($_POST['BI_annual_self_consumption'], 2, ',', '.') : ''; ?>">

        <label class="label-classic">x Einspeisevergütung gemäß EVU-Vertrag in € / kWh</label>
        <input type="number" id="BI_self_consumption_tariff" name="BI_self_consumption_tariff" step="0.01" oninput="calculate_BI_Total()" value="<?php echo isset($_POST['BI_self_consumption_tariff']) ? number_format($_POST['BI_self_consumption_tariff'], 2, ',', '.') : ''; ?>">
    </div>
</div>
<br><br>
...
<div class="subsection-save">
        ...
        Durch Absenden dieses Formulars wird bestätigt, dass sämtliche Fragen wahrheitsgetreu beantwortet wurden. Unrichtige Angaben können einen Deckungsverlust zur Folge haben.</span>
    
    <input type="submit" value="Senden"><br>

    </form>
    <div class="center">
        <a href="index.php">Formular leeren</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="load.php">Gespeicherten Antrag forsetzen</a>
    </div>
