...
$age = $_POST['age'];
$theft_fence = $_POST['theft_fence'];
$theft_video = $_POST['theft_video'];

$coinsured = $_POST['coinsured'] ?? $form['coinsured'];

$coinsured_company = $_POST['coinsured_company'] ?? 'NIEN';

$coinsured_street = $_POST['coinsured_street'] ?? 'NIEN';

$coinsured_postalcode = $_POST['coinsured_postalcode'] ?? 'NIEN';

$coinsured_place = $_POST['coinsured_place'] ?? 'NIEN';

$coinsured_function = $_POST['coinsured_function'] ?? 'NIEN';

$hold = $_POST['hold'] ?? '';

$hold_street = $_POST['hold_street'] ?? '';

$hold_creditor = $_POST['hold_creditor'] ?? '';

$hold_postalcode = $_POST['hold_postalcode'] ?? '';
...
$sql = "UPDATE `pv_plants` 
        ...
        SET `applicant` = ?, `street` = ?, `postalcode` = ?, `place` = ?, `tracker` = ?, 
            ...
            `age` = ?, `theft_fence` = ?, `theft_video` = ?,  `coinsured` = ?,
            
            `coinsured_company` = ?,  `coinsured_street` = ?,
            
            `coinsured_postalcode` = ?, `coinsured_place` = ?,
...
if ($stmt->execute()) {
    updateRestOfPvPlant($id);
    if($isCompleted){
        $_SESSION['success_message'] = "Daten erfolgreich aktualisiert.";
        sendEmailInCompleted($_SESSION['email']);
        sendEmailInCompleted('service@surendo.com');
        session_write_close();
    }
...
function updateRestOfPvPlant($pv_plant_id)
{
    global $conn;

    $predamage = $_POST["predamage"] ?? '';
    $predamage_details = $_POST["predamage_details"] ?? '';
    $predamage_refusal = $_POST["predamage_refusal"] ?? '';
    $risk_description_expert = $_POST["risk_description_expert"] ?? '';
    $risk_description_prototype = $_POST["risk_description_prototype"] ?? '';
    $risk_description_maintenance = $_POST["risk_description_maintenance"] ?? '';
    $risk_description_lightning = $_POST["risk_description_lightning"] ?? '';
    $risk_description_inverter_protection = $_POST["risk_description_inverter_protection"] ?? '';
    $risk_description_combustible_substructure = $_POST["risk_description_combustible_substructure"] ?? '';
    $risk_description_uninhabited_area = $_POST["risk_description_uninhabited_area"] ?? '';
    $risk_description_fire_exposed_area = $_POST["risk_description_fire_exposed_area"] ?? '';
    $risk_description_wildfire = $_POST["risk_description_wildfire"] ?? '';
    $risk_description_accessibility = $_POST["risk_description_accessibility"] ?? '';
    $risk_description_windpower = $_POST["risk_description_windpower"] ?? '';
    $risk_description_official_restriction = $_POST["risk_description_official_restriction"] ?? '';
    $risk_description_official_restriction_details = $_POST["risk_description_official_restriction_details"] ?? '';
    $risk_description_statics = $_POST["risk_description_statics"] ?? '';
    $risk_description_certified = $_POST["risk_description_certified"] ?? '';
    $risk_description_flexible_modules = $_POST["risk_description_flexible_modules"] ?? '';
    $risk_description_cable = $_POST["risk_description_cable"] ?? '';
    $charging_station = $_POST["charging_station"] ?? '';
    $battery_storage = $_POST["battery_storage"] ?? '';
    $eur_charging_station = $_POST["eur_charging_station"] ?? '';
    $eur_battery_storage = $_POST["eur_battery_storage"] ?? '';
    $risk_description_charging_station_prototype = $_POST["risk_description_charging_station_prototype"] ?? '';
    $risk_description_charging_station_point = $_POST["risk_description_charging_station_point"] ?? '';
    $risk_description_charging_station_standards = $_POST["risk_description_charging_station_standards"] ?? '';
    $risk_description_charging_station_protection = $_POST["risk_description_charging_station_protection"] ?? '';
    $risk_description_battery_storage_discharge_protection = $_POST["risk_description_battery_storage_discharge_protection"] ?? '';
    $plant_type = $_POST["plant_type"] ?? '';
    $ground_plant_elevated = $_POST["ground_plant_elevated"] ?? '';
    $ground_plant_underground = $_POST["ground_plant_underground"] ?? '';
    $roof_plant_amount_buildings = $_POST["roof_plant_amount_buildings"] ?? '';
    $roof_plant_usage = $_POST["roof_plant_usage"] ?? '';
    $roof_plant_eaves_height = $_POST["roof_plant_eaves_height"] ?? '';
    $roof_plant_soft_roofing = $_POST["roof_plant_soft_roofing"] ?? '';
    $roof_plant_soft_walls = $_POST["roof_plant_soft_walls"] ?? '';
    $roof_plant_mechanical_protection = $_POST["roof_plant_mechanical_protection"] ?? '';
    $roof_plant_animal_farm = $_POST["roof_plant_animal_farm"] ?? '';
    $roof_plant_fire_wall = $_POST["roof_plant_fire_wall"] ?? '';
    $roof_plant_collapse = $_POST["roof_plant_collapse"] ?? '';
    $roof_plant_snow_removal = $_POST["roof_plant_snow_removal"] ?? '';
    $roof_plant_gravel = $_POST["roof_plant_gravel"] ?? '';
    $roof_plant_contracting = $_POST["roof_plant_contracting"] ?? '';
    $indemnify_VAT = $_POST['indemnify_VAT'] ?? '';
    $load_break_switch = $_POST['load_break_switch'] ?? '';



    if (!$pv_plant_id) {
        die("Error: 'pv_plant_id' is required for updating the record.");
    }

    $sql = 'UPDATE pv_plants_1 SET 
                predamage = ?,
                predamage_details = ?,
                predamage_refusal = ?,
                risk_description_expert = ?,
                risk_description_prototype = ?,
                risk_description_maintenance = ?,
                risk_description_lightning = ?,
                risk_description_inverter_protection = ?,
                risk_description_combustible_substructure = ?,
                risk_description_uninhabited_area = ?,
                risk_description_fire_exposed_area = ?,
                risk_description_wildfire = ?,
                risk_description_accessibility = ?,
                risk_description_windpower = ?,
                risk_description_official_restriction = ?,
                risk_description_official_restriction_details = ?,
                risk_description_statics = ?,
                risk_description_certified = ?,
                ...
                eur_battery_storage = ?,
                risk_description_charging_station_prototype = ?,
                risk_description_charging_station_point = ?,
                risk_description_charging_station_standards = ?,
                risk_description_charging_station_protection = ?,
                risk_description_battery_storage_discharge_protection = ?,
                plant_type = ?,
    ...
    if ($stmt->execute()) {
        ...
        die("Error updating pv_plant_1 data: " . $stmt->error);
    }
}

function sendEmailInCompleted($email)
{
    ...
    // Betreff der E-Mail
    $mail->Subject = 'Surendo.com – Ihr Projekt ist abgeschlossen';
    ...
    <div style="font-family: Arial, sans-serif; background-color: #cfe9ee7c; padding: 20px; border: 0px; border-radius: 12px; max-width: 500px; margin: 0 auto;">
        <h2 style="color: #113388; text-align: center;">Projekt ist abgeschlossen</h2>
        <p style="font-size: 16px; color: #333; text-align: center;">
            Ihr Projekt mit der ID-Nummer ' . htmlspecialchars($_POST["form_id"]) . ' wurde erfolgreich abgeschlossen.
        </p>
        <p style="font-size: 24px; color: #333; font-weight: bold; text-align: center;">
            <p><strong>' . htmlspecialchars($_POST["applicant"]) . '</strong>, ' . htmlspecialchars($_POST["street"]) . ', ' . htmlspecialchars($_POST["postalcode"]) . ', ' . htmlspecialchars($_POST["place"]) . '</p>
            <p><strong>Versicherungsbeginn:</strong> ' . htmlspecialchars($_POST["date_commencement"]) . '</p>
        </p>';

    if ($_POST["address_or_coordinates"] == "address") {
        $mail->Body .= '
        <p><strong>Straße und Hausnummer:</strong> ' . htmlspecialchars($_POST["address_street"]) . '</p>
        <p><strong>Postleitzahl und Ort:</strong> ' . htmlspecialchars($_POST["address_postalcode"]) . ' ' . htmlspecialchars($_POST["address_place"]) . '</p>';
    } else {
        $mail->Body .= '
        <p><strong>Ost/Nord-Koordinaten (Dezimalgrad z.B. 2.17403 41.40338):</strong> ' . htmlspecialchars($_POST["coordinates"]) . '</p>';
    }

    if ($_POST["insured_land"] == "Ausland") {
        $mail->Body .= '
        <p><strong>Land:</strong> ' . htmlspecialchars($_POST["insured_land"]) . '</p>
        <p><strong>Name des Landes:</strong> ' . htmlspecialchars($_POST["name_other_land"]) . '</p>';
    } else {
        $mail->Body .= '
        <p><strong>Land:</strong> ' . htmlspecialchars($_POST["insured_land"]) . '</p>';
    }
